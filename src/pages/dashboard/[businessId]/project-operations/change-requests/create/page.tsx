...
export default function CreateChangeRequestPage() {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      const [projRes, usersRes, custRes, contRes] = await Promise.all([
        projectOperationsAPI.getProjects(businessId as string).catch(() => ({})),
        usersAPI.getBusinessUsers(businessId as string).catch(() => ({})),
        contactsAPI.getCustomers(businessId as string).catch(() => ({})),
        contactsAPI.getContacts(businessId as string).catch(() => ({}))
      ]);
      ...
      if ((custRes as any).success) setCustomers((custRes as any).customers || (custRes as any).data || []);
  ...
  const handleSubmit = async (isDraft: boolean = false) => {
    ...
    try {
      ...
      const res = await projectOperationsAPI.createChangeRequest(businessId as string, payload);
