...
export const projectOperationsAPI = {
  async getProjects(businessId: string): Promise<any> {
    ...
    const response = await fetch(`${API_ROOT}/project-operations`, {
      method: 'GET',
      headers: {
  ...
  async createChangeRequest(businessId: string, payload: any): Promise<any> {
    const token = getCookie('token') || getCookie('accessToken');
    const response = await fetch(`${API_ROOT}/project-operations/${payload.projectId}/change-requests`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
  ...
  async getRequirements(businessId: string, customerId?: string, status?: string): Promise<any> {
    ...
    if (customerId) append('customerId', customerId);
  ...
  async getEstimations(businessId: string, requirementId?: string, customerId?: string, status?: string): Promise<any> {
    ...
    const params = new URLSearchParams();
    if (requirementId) append('requirementId', requirementId);
    if (customerId) append('customerId', customerId);
    if (status) append('status', status);
  ...
  async getWarranties(businessId: string, customerId?: string, projectId?: string) {
    ...
    const params = new URLSearchParams();
    if (customerId) append('customerId', customerId);
    if (projectId) append('projectId', projectId);
    const qs = toString() ? `?${toString()}` : '';
