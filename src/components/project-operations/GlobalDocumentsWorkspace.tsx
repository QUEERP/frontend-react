...
export function GlobalDocumentsWorkspace({ businessId }: { businessId: string }) {
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      if (debouncedSearch) append('search', debouncedSearch);
      if (activeFolder) append('folder', activeFolder);
      if (fileTypeFilter) append('fileType', fileTypeFilter);
  ...
  const handleFileUpload = useCallback(async (files: File[]) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        setDocuments(prev => [{ ...payload, id: Math.random().toString(36).substr(2, 9), createdAt: new Date().toISOString() }, ...prev]);
