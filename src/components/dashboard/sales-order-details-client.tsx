                  ...
                  <tbody>
                    {order.items?.map((item) => (
                      <tr key={item.id} className="border-b last:border-0 hover:bg-muted/10 transition-colors">
                        ...
                        <td className="p-3 text-right font-mono">{formatCurrency(item.price, order.currency)}</td>
                        <td className="p-3 text-right font-mono text-muted-foreground">{item.taxPercent ? `${item.taxPercent}%` : '0%'}</td>
                        <td className="p-3 text-right font-mono font-semibold">{formatCurrency(item.total, order.currency)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="mt-6 flex justify-end">
                <div className="w-full max-w-xs space-y-2 text-sm">
                  ...
                  {order.tax !== undefined && order.tax !== null && order.tax > 0 && (
                    <div className="flex justify-between text-muted-foreground">
                      <span>Tax</span>
                      <span className="font-mono">{formatCurrency(order.tax, order.currency)}</span>
                    </div>
                  )}
                  {order.discount !== undefined && order.discount !== null && order.discount > 0 && (
                    <div className="flex justify-between text-rose-600">
                      <span>Discount</span>
                      <span className="font-mono">-{formatCurrency(order.discount, order.currency)}</span>
                    </div>
                  )}
                  <div className="flex justify-between border-t pt-2 text-base font-bold text-foreground">
                    <span>Total Amount</span>
                    <span className="font-mono text-primary">{formatCurrency(order.totalAmount, order.currency)}</span>
                  </div>
                </div>
              </div>
            </CardContent>
            ...
            <CardContent className="pt-4 space-y-3 text-sm">
              ...
              {order.quotation && (
                <div className="flex justify-between">
                  ...
                  <span className="text-primary font-medium">{order.quotation.quoteNumber}</span>
                </div>
              )}
            </CardContent>
          </Card>
