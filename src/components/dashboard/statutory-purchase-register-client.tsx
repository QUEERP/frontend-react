...
import { exportToExcel, exportPurchaseRegisterPdf, printPage } from '@/lib/export-utils'
...
export default function StatutoryPurchaseRegisterClient() {
  ...
  return (
    <div className="p-4 sm:p-8 max-w-7xl mx-auto space-y-8 bg-muted/50 dark:bg-slate-900/50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">Purchase Register</h1>
          <p className="text-muted-foreground mt-1">
            {isIndia ? 'Q2 FY2025-26' : 'Q2 2025'} &mdash; {filtered.length} bills &mdash; Total: {curr} {fmt(totals.total)}
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm" className="text-emerald-600 border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700"
            onClick={() => exportToExcel(filtered, 'Purchase_Register')}>
            <FileSpreadsheetIcon className="size-4 mr-1" />Excel
          </Button>
          <Button size="sm" className="bg-rose-600 hover:bg-rose-700 text-white"
            onClick={() => exportPurchaseRegisterPdf(filtered, isIndia ? 'Q2 FY2025-26' : 'Q2 2025')}>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <Card className="border-none shadow-sm bg-card dark:bg-slate-900">
          <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div>
              <CardTitle>Bill Register</CardTitle>
              <CardDescription>All purchase bills for the selected period</CardDescription>
            </div>
            <div className="relative w-full sm:w-72">
              <SearchIcon className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-muted-foreground" />
              <Input
                placeholder={`Search bill, vendor, ${taxIdLabel}...`}
