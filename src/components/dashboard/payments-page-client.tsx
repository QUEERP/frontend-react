    ...
    const fetchPayments = async () => {
      ...
      try {
        ...
        const rows = paymentList.map((p: any) => {
          ...
          let refNumber = proj?.projectCode || p.invoice?.invoiceNumber || p.quotation?.quoteNumber || '-';
                  ...
                  <TableRow key={payment.id} className="group hover:bg-muted/50 border-b border-border transition-colors">
                    <TableCell className="py-4">
                      <div className="flex flex-col">
                        ...
                        {payment.projectName && (
                          <span className="text-[11px] text-muted-foreground font-medium mt-0.5 truncate max-w-[150px]" title={payment.projectName}>
                            {payment.projectName}
                          </span>
                        )}
                      </div>
