...
export function DashboardPageClient({ businessId }: { businessId: string }) {
  ...
  const [fetchedInvoices, setFetchedInvoices] = React.useState<any[]>([])
  ...
  React.useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        if (invData?.success) setFetchedInvoices(invData.data || invData.invoices || [])
        if (expData?.success) setFetchedExpenses(expData.data || expData.expenses || [])
      } catch (e) {
  ...
  const invoices = React.useMemo(() => {
    const bizInvoices = Array.isArray(business?.invoices) ? business.invoices : []
    return bizInvoices.length >= fetchedInvoices.length ? bizInvoices : fetchedInvoices
  }, [business, fetchedInvoices])

  const expenses = React.useMemo(() => {
    const bizExpenses = Array.isArray(business?.expenses) ? business.expenses : []
    return bizExpenses.length >= fetchedExpenses.length ? bizExpenses : fetchedExpenses
  }, [business, fetchedExpenses])
  ...
  const monthlyChartData = React.useMemo(() => {
    ...
    const map = new Map<string, { month: string; billed: number; collected: number; expenses: number }>()



    for (const item of months) {

      map.set(item.key, { month: item.label, billed: 0, collected: 0, expenses: 0 })
    ...
    for (const exp of expenses) {
      ...
      const row = map.get(key)
      if (!row) continue
      row.expenses += Number(exp.amount || 0)
    }



    return months.map((item) => map.get(item.key) || { month: item.label, billed: 0, collected: 0, expenses: 0 })
  ...
  const expensePieData = React.useMemo(() => {
    ...
    const COLORS = ['#8ea2ff', '#4fe0c1', '#ffc66b', '#ff7b72', '#b392f0', '#7ee787'];
        ...
        <div className="grid gap-4 lg:grid-cols-3 mb-4">
          <Card className="lg:col-span-2 min-w-0 rounded-2xl border-border bg-card shadow-[0_2px_10px_rgba(0,0,0,0.02)] dark:border-[#23272c] dark:bg-[#181a20]">
            <CardHeader className="pb-4 pt-6">
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-base font-bold text-foreground dark:text-slate-100">Income vs Expenses</CardTitle>
                  <CardDescription className="text-[13px] mt-1 text-muted-foreground dark:text-slate-400">Total Billed vs Total Expenses (Last 6 months)</CardDescription>
                </div>
                <div className="flex size-8 items-center justify-center rounded-full bg-muted dark:bg-[#1c2128]">
                  <TrendingUpIcon className="size-4 text-muted-foreground dark:text-slate-400" />
                </div>
          ...
          <Card className="min-w-0 rounded-2xl border-border bg-card shadow-[0_2px_10px_rgba(0,0,0,0.02)] dark:border-[#23272c] dark:bg-[#181a20]">
            <CardHeader className="pb-4 pt-6">
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-base font-bold text-foreground dark:text-slate-100">Expenses by Category</CardTitle>
                  <CardDescription className="text-[13px] mt-1 text-muted-foreground dark:text-slate-400">Distribution of company expenses</CardDescription>
                </div>
                <div className="flex size-8 items-center justify-center rounded-full bg-muted dark:bg-[#1c2128]">
            ...
            <CardContent className="pb-6">
              {expensePieData.length === 0 ? (
                ...
                <ChartContainer config={chartConfig} className="h-72 w-full">
                  <PieChart>
                    <ChartTooltip cursor={false} content={<ChartTooltipContent />} />
                    <Pie data={expensePieData} dataKey="value" nameKey="name" innerRadius={60} outerRadius={80} paddingAngle={5}>
